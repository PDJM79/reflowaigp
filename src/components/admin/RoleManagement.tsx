...
function AddRoleForm({ saving, onAdd }: AddRoleFormProps) {
  ...
  const handleAdd = () => {
    ...
    onAdd(name, email, role as UserRole);
    setName('');
    setEmail('');
    setRole('');
  };
  ...
  return (
    <div className="border rounded-lg p-4 space-y-4">
      ...
      <Button onClick={handleAdd} disabled={saving} className="w-full">
        {saving
          ? <div className="flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Adding...</div>
...
function AssignmentList({ assignments, onDelete }: AssignmentListProps) {
        ...
        <div key={assignment.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-accent/50 transition-colors">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {assignment.user_id && <Badge variant="secondary">Active User</Badge>}
            </div>
          </div>
          <Button
            variant="ghost"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Manage roles and designations for your practice</DialogDescription>
        </DialogHeader>
        {loading ? (
          ...
          <ScrollArea className="flex-1 pr-4">
            <div className="space-y-6">
              <AddRoleForm saving={saving} onAdd={addRoleAssignment} />
              <div className="space-y-4">
                <h3 className="text-sm font-medium">Current Role Assignments ({roleAssignments.length})</h3>
                <AssignmentList assignments={roleAssignments} onDelete={deleteRoleAssignment} />
              </div>
            </div>
          </ScrollArea>
        )}
